    ...
    for filename in os.listdir(folder_path):
        if re.match(r"Subject_\d+_T4_Meaningful_Text_metrics\.csv", filename):
            files_metrics.append(os.path.join(folder_path, filename))
                ...
                r"Subject_\d+_T4_Meaningful_Text_fixations\.csv",
                filename):
...
def process_dataset(files_metrics, files_fixations):
    ...
    for f in files_metrics:
        ...
        fix_line_ratio = row.get("n_fix_trial") / row.get("n_lines_fixated")
        row["fixation_per_line_ratio"] = fix_line_ratio
    ...
    dataset = dataset.merge(
        labels[["subject_id", "class_id"]],
        left_on="sid",
        right_on="subject_id").drop(columns=["subject_id"])
