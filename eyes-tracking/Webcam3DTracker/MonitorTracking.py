...
def on_press(key):
    global mouse_control_enabled, f7_was_pressed

    # --- HOLD KEYS (j, k, i, l, etc.) ---
    try:
        if key.char:
    ...
    # --- SPECIAL KEYS (F-keys, arrows, etc.) ---
    if key == keyboard.Key.f7:
        if not f7_was_pressed:  # edge detection
